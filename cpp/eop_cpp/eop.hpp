...
LHSlt<LHS, decltype(typeEquality)> operator<(const LHS& lhs, decltype(typeEquality))
{
    return { lhs };
}
...
public:
    ...
    template <typename L, typename R>
    void ValueTypeIs(const L& l, const R& r)
    {
        ...
        std::cout << (!repImplieseqHolds ? "ambigious" : "") << and_s << (!eqImpliesrepHolds ? "not uniquely represented" : "") << "\n";
    }
